...
export default function AdminOrderProcessed () {
    ...
    const onDeleteOrder = (id) => {
        deleteOrder(id, (data) => {
            console.log(data);
            GoToPage('/admin/order/processed');
        })
    }

    useEffect(() => {
        getOrders((response) => {
            setOrders(response.data.orders.filter(order => order.currentStatus.map(item => item.status.status)[order.currentStatus.map(item => item.status.status).length - 1] === 'On Process' || order.currentStatus.map(item => item.status.status)[order.currentStatus.map(item => item.status.status).length - 1] === 'On Packing' ));
            console.dir(
        ...
        <DashboardFragment>
            ...
            <div className="mb-4 dark:bg-gray-800">
                    <div className="bg-white rounded-xl shadow-md p-7">
                        <h1 className='font-medium text-gray-500 text-2xl my-5'>On Processed Orders</h1>
