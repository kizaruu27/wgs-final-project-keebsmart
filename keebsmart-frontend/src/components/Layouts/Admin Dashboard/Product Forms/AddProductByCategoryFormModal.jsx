...
export default function AddProductByCategoryFormModal({openModal, setOpenModal, catId, categoryName}) {
    ...
    useEffect(() => {
        getUnusedInventory((data) => {
            // console.log(data.filter(item => item.categoryId === catId));
            setInventory(data.filter(item => item.categoryId === catId));
        })
    }, []);
    ...
    const postNewProduct = (e) => {
        ...
        addNewProduct(inventoryId, productName, description, brand, categoryId, specs, imagePreview, images, (data) => {
            GoToPage(`/admin/products/${categoryName}`)
        }, (error) => {
            console.error(error);
        })
    }
