import { addNewProduct, getAllCategory } from "../../server/productController";
import { useState, useEffect } from "react";
import { GoToPage } from "../../server/pageController";

export default function ModalFormAddProduct({onRedirect, catId}) {
    ...
    const onSuccessAddProduct = () => GoToPage(onRedirect, 100);
    const onFailedAddProduct = (error) => console.log(error);

    const postNewProduct = (e) => {
        e.preventDefault();
        addNewProduct(productName, description, brand, catId, imagePreview, images, onSuccessAddProduct, onFailedAddProduct);
    }
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 sm:gap-6">
                            ...
                            <div>
                                ...
                                <select onChange={e => setCategoryId(e.target.value)} id="category" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5">
                                    {categories.filter(cat => cat.id === catId).map((item, key) => (
                                        <option key={key} defaultValue={catId}>{item.categoryName}</option>
                                    ))}
                                </select>
                            </div>
                            <div className="sm:col-span-2">
